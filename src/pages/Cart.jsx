...
function Cart() {
  ...
  useEffect(()=>{
    if(cartItems?.length>0){
      setTotal(cartItems?.map(item=>item.totalPrice).reduce((t1,t2)=>t1+t2))
    }else{
      setTotal(0)
    }
  ...
  const handlecheckout = ()=>{
    dispatch(emptyCart())
    toast.success('Order successfully... thank you for purchasing')
           ...
           <div className='container' style={{marginTop:'100px'}}>
            {
                ...
                <div className="row mt-5">
                  <div className="col-lg-8">
                    ...
                    <div className='float-end mt-3'>
                      <button onClick={()=>dispatch(emptyCart())} className='btn btn-primary ms-2'>EMPTY CART</button>
                      <Link className='btn btn-danger ms-2' to={'/'}>Shop More</Link>
                    </div>
                  </div>
                  <div className="col-lg-4">
                    <div className='shadow border rounded p-4'>
                      <h5>Total Product: <b className='text-danger'>{cartItems?.length}</b></h5>
              ...
              <img className='img-fluid' width={'400px'} src="https://img.freepik.com/premium-vector/shopping-cart-with-cross-mark-wireless-paymant-icon-shopping-bag-failure-paymant-sign-online-shopping-vector_662353-912.jpg" alt="" />
